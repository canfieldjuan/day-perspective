...
@pytest.mark.integration
def test_random_enriched_date_never_offers_a_journey_to_nowhere(
    session: Session, tmp_path: Path, reviewed_un_wpp: None
) -> None:
    """The control exists to escape repetition. With no enriched date the
    honest answer is 404 so the interface can hide it, not a context date
    dressed up as evidence."""
    from fastapi.testclient import TestClient

    from app.coverage import random_enriched_date
    from app.database import get_session
    from app.main import app

    store = LocalFilesystemPublishedProfileStore(tmp_path / "published")
    context_dates = [date(1986, 2, day) for day in range(1, 4)]
    run = start_batch_run(
        session,
        kind=CONTEXT_BATCH_KIND,
        requested={"dates": [value.isoformat() for value in context_dates]},
    ...
    try:
        client = TestClient(app)
        empty = client.get("/api/v1/coverage/enriched/random")
        assert empty.status_code == 404
        assert empty.json()["status"] == "no_enriched_dates"

        publish_enriched(session, store, date(1986, 3, 3), label="random-enriched")
        session.commit()
        rebuild_coverage_index(session, store=store)
        session.commit()

        found = client.get("/api/v1/coverage/enriched/random")
    finally:
        app.dependency_overrides.clear()

    assert found.status_code == 200
    assert found.json() == {"status": "enriched_date", "date": "1986-03-03"}
    # The literal path must not be swallowed by /coverage/{profile_date}.
    assert random_enriched_date(session) == date(1986, 3, 3)
