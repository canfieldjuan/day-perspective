...
| `golden_set.publication_status` | `not_generated`, `context_published`, `published_and_validated` | Only `published_and_validated` counts toward release readiness; `context_published` records that the canary generated and validated a profile, not that a human reviewed it (D035). |
