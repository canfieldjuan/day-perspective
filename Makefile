...
.PHONY: help install web-install api-install db-up db-down db-reset local-storage-reset clean-reset db-migrate seed-timezones seed api-migrate api-seed ingest-usgs-fixture ingest-usgs-live ingest-usgs-dry-run review-usgs-fixture ingest-un-wpp-fixture ingest-un-wpp-live ingest-un-wpp-dry-run review-un-wpp-fixture ingest-ucdp-annual-fixture ingest-ucdp-annual-live review-ucdp-annual-fixture review-ucdp-annual-all-years ingest-ucdp-ged-fixture review-ucdp-ged-fixture ingest-wikidata-fixture ingest-wikidata-live ingest-wikidata-dry-run enrich-wikidata resolve-wikidata validate-golden-set reconcile-publications reconcile-publications-repair publish-context-year publish-context-resume publish-context-retry-failed rebuild-coverage golden-canary publish-archive publish-golden api api-run api-test api-lint api-typecheck contracts-test contracts-lint contracts-typecheck web web-run web-test web-lint web-typecheck web-e2e web-e2e-full-stack web-build test-integration test-e2e build check audit verify

help:
	@printf '%s\n' 'Targets: install, db-up, db-down, db-reset, clean-reset, db-migrate, seed, ingest-usgs-fixture, ingest-usgs-live, ingest-usgs-dry-run, review-usgs-fixture, ingest-un-wpp-fixture, ingest-un-wpp-live, ingest-un-wpp-dry-run, review-un-wpp-fixture, ingest-ucdp-annual-fixture, ingest-ucdp-annual-live, review-ucdp-annual-fixture, review-ucdp-annual-all-years, ingest-ucdp-ged-fixture, review-ucdp-ged-fixture, ingest-wikidata-fixture, ingest-wikidata-live, ingest-wikidata-dry-run, publish-golden, golden-canary, publish-archive, rebuild-coverage, reconcile-publications, api, web, test-integration, test-e2e, build, check, verify'
...
seed-timezones:
	cd services/api && DATABASE_URL='$(DATABASE_URL)' $(API_PYTHON) -m app.timezone_cli seed
...
ingest-ucdp-annual-live:
	@# Operator-invoked outbound fetch of the pinned UCDP/PRIO v26.1
	@# release. Never run in CI: CI stays offline on the committed 1964
	@# excerpt. The raw file lands in the local raw-source store, outside
	@# Git; only its metadata and checksum are committed.
	cd services/api && DATABASE_URL='$(DATABASE_URL)' RAW_SOURCE_ROOT='$(PROJECT_ROOT)/.local/raw-sources' $(API_PYTHON) -m app.context_cli ingest-ucdp-annual --live
...
ingest-wikidata-fixture:
	cd services/api && DATABASE_URL='$(DATABASE_URL)' RAW_SOURCE_ROOT='$(PROJECT_ROOT)/.local/raw-sources' $(API_PYTHON) -m app.candidate_cli ingest --fixture '$(PROJECT_ROOT)/data/fixtures/wikidata/Q749610.json'

ingest-wikidata-live:
	cd services/api && DATABASE_URL='$(DATABASE_URL)' RAW_SOURCE_ROOT='$(PROJECT_ROOT)/.local/raw-sources' $(API_PYTHON) -m app.candidate_cli ingest $(ARGS)

ingest-wikidata-dry-run:
	cd services/api && DATABASE_URL='$(DATABASE_URL)' RAW_SOURCE_ROOT='$(PROJECT_ROOT)/.local/raw-sources' $(API_PYTHON) -m app.candidate_cli ingest --fixture '$(PROJECT_ROOT)/data/fixtures/wikidata/Q749610.json' --dry-run

enrich-wikidata:
	cd services/api && DATABASE_URL='$(DATABASE_URL)' PUBLISHED_PROFILE_ROOT='$(PROJECT_ROOT)/.local/published-profiles' $(API_PYTHON) -m app.candidate_cli enrich
